...
interface DeltaDao {
    ...
    @Query("SELECT COUNT(*) from deltas WHERE synced = 0 AND modelName = :modelName AND action = :action")
    fun unsyncedCount(modelName: Delta.ModelName, action: Delta.Action): Flowable<Int>

    @Query("SELECT COUNT(*) from deltas WHERE synced = 0 AND modelName = :modelName")
